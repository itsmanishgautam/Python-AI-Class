...
# Function to get CSRF token (security measure)
...
def extract_table_data(html):
    # Regular expressions to find table headers and body
    ...
    # Extract data from headers and rows
    head_data_all = []
...
# Function to parse table data into a dictionary format
def parse_table_data(head_data_all, matches_data_all):
    num_columns = len(head_data_all)
    data = []

    # Separate data into rows (sublists)
    ...
    # Function to extract symbol from HTML
    def extract_symbol(html):
        name_match = re.search(r'>([^<]+)<', html)
        return name_match.group(1) if name_match else None

    # Check for "No Record Found" case
    if sublists == [[' No Record Found.']]:
        return "No Record Found", data

    # Loop through sublists and create dictionaries for each entry
...
# # Function to write data to CSV file (commented out for now)
